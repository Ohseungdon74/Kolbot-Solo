...
function LoadConfig () {
	...
	/* ### leeching section ###
	* Unless stated otherwise, leader's character name isn't needed on order to run. 달리 명시되지 않는 한, 리더의 캐릭터 이름은 달리기 위해 필요하지 않습니다.
	* Don't use more scripts of the same type! (Run AutoBaal OR BaalHelper, not both) 같은 유형의 스크립트를 더 사용하지 마십시오! (둘 다가 아닌 AutoBaal 또는 BaalHelper 실행)
	*/

	Config.Leader = ""; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper), 	리더의 게임 내 캐릭터 이름. 자동 감지를 시도하려면 공백으로 두십시오 (AutoBaal, Wakka, MFHelper에서 작동).
	Config.QuitList = [""]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"]; 	종료 할 문자 이름 목록입니다. 예 : Config.QuitList = [ "MySorc", "MyDin"];
	Config.QuitListMode = 0; // 0 = use character names; 1 = use profile names (all profiles must run on the same computer).	캐릭터 이름을 사용하십시오. 1 = 프로필 이름 사용 (모든 프로필이 동일한 컴퓨터에서 실행되어야 함).
	Config.QuitListDelay = [5, 10]; // Quit the game with random delay in case of using Config.QuitList. Example: Config.QuitListDelay = [1, 10]; will exit with random delay between 1 and 10 seconds.
								// Config.QuitList를 사용하는 경우 무작위 지연으로 게임을 종료합니다. 예 : Config.QuitListDelay = [1, 10]; 1 초에서 10 초 사이의 임의 지연으로 종료됩니다.

	Scripts.TristramLeech = false; // Enters Tristram, attempts to stay close to the leader and will try and help kill.	Tristram에 들어가 리더에게 가까이 머물러 살인을 도우려고합니다.
	Scripts.TravincalLeech = false; // Enters portal at back of Travincal.	Travincal 뒤쪽에있는 포털로 들어갑니다.
		Config.TravincalLeech.Helper = true; // If set to true the character will teleport to the stairs and help attack.	True로 설정하면 캐릭터가 계단으로 순간 이동하여 공격을 돕습니다.
	Scripts.MFHelper = false; // Run the same MF run as the MFLeader. Leader must have Config.MFLeader = true,	MFLeader와 동일한 MF 실행을 실행합니다. 리더는 Config.MFLeader = true 여야합니다.
	Scripts.Wakka = true; // Walking chaos leecher with auto leader assignment, stays at safe distance from the leader,	자동 리더 할당 기능이있는 워킹 카오스 리처, 리더로부터 안전한 거리 유지
	Scripts.SealLeecher = false; // Enter safe portals to Chaos. Leader should run SealLeader.	카오스로가는 안전한 포털에 들어가십시오. 리더는 SealLeader를 실행해야합니다.
	Scripts.DiabloHelper = false; // Chaos helper, kills monsters and doesn't open seals on its own.	카오스 도우미, 몬스터를 죽이고 자체적으로 씰을 열지 않습니다.
	...
	Scripts.ChestMania = false; // Open chests in configured areas. See sdk/areas.txt
		...
		Config.ChestMania.Act4 = [108]; // List of act 4 areas to open chests in
		//Config.ChestMania.Act5 = [115, 116, 119, 125, 126, 127]; // List of act 5 areas to open chests in
		Config.ChestMania.Act5 = [131];
	// 해당 지역 몹을 정리 합니다.. Config.ClearType을 사용하여 죽일 몬스터 유형을 결정합니다.
	Scripts.ClearAnyArea = false; // Clear any area. Uses Config.ClearType to determine which type of monsters to kill.
		Config.ClearAnyArea.AreaList = [12, 16, 20, 21, 22, 23, 24, 25, 34, 35, 36, 83, 100, 101, 102, 108, 128, 129, 130, 131]; // List of area ids to clear. See sdk/areas.txt
	...
	// Potion settings
	Config.UseHP = 75; // Drink a healing potion if life is under designated percent.	생명력이 지정된 비율 미만이면 치유 물약을 마 십니다.
	...
	/* Minimum amount of potions. If we have less, go to vendor to purchase more.	물약의 최소량. 부족한 경우 공급 업체로 이동하여 더 구입하십시오.
	 * Set rejuvenation columns to 0, because they can't be bought.					rejuvenation 열은 구매할 수 없으므로 0으로 설정합니다.
	 */
	...
	// Pickit 구성. 기본 폴더는 kolbot / pickit입니다.
	...
	//Config.PickitFiles.push("classic.nip");
	
	Config.PickRange = 40; // Pick radius	###	반경 선택
	Config.FastPick = true; // Check and pick items between attacks ###	공격을 쉬는동안 아이템 확인
	...
	// Item identification settings 아이템 확인 후 버릴것인지 체크
	Config.CainID.Enable = true; // Identify items at Cain(케인에게 아이템 식별)
	Config.CainID.MinGold = 10000; // Minimum gold (stash + character) to have in order to use Cain.
	Config.CainID.MinUnids = 3; // Minimum number of unid items in order to use Cain. 케인에게 확인 할 최소 아이템 아이템 수.
	Config.FieldID = true; // Identify items in the field instead of going to town.(마을에가지 않고 물건을 확인)
	Config.DroppedItemsAnnounce.Enable = true;	// Announce Dropped Items to in-game newbs ### 게임내에서 팝업에 드랍 목록 노출
	Config.DroppedItemsAnnounce.Quality = []; // Quality of item to announce. See NTItemAlias.dbl for values. Example: Config.DroppedItemsAnnounce.Quality = [6, 7, 8];

	// Manager Item Log Screen 아이템 정보 로그 띄우기
	...
	Config.LastMessage = "다음 방 이동 : $nextgame"; // Message or array of messages to say at the end of the run. Use $nextgame to say next game - "Next game: $nextgame" (works with lead entry point)
	...
	Config.MiniShopBot = false; // Scan items in NPC shops.	#NPC 아이템 스캔 
	Config.PacketShopping = false; // Use packets to shop. Improves shopping speed.
	Config.TownCheck = false; // Go to town if out of potions	# 물약이 없으면 마을로 이동
	Config.LogExperience = false; // Print experience statistics in the manager.	###	관리자에서 경험 통계를 인쇄합니다.
	...
	// MF Switch ###	보스 사냥시 무기 스왑 %
	Config.MFSwitchPercent = 20; // Boss life % to switch to secondary weapon slot. Set to 0 to disable.
	 ...
	 */
	Config.AttackSkill[0] = 55; // Preattack skill. Not implemented yet. 파볼오브소서[55, 64, 42, 64, 47, 47, 54];
	Config.AttackSkill[1] = 64; // Primary skill to bosses. 쿨타임이 있는 스킬
	Config.AttackSkill[2] = 42; // Primary untimed skill to bosses. Keep at -1 if Config.AttackSkill[1] is untimed skill. 1번 스킬 발동 후 딜레이시 스킬 시전 할 스킬
	...
	Config.AttackSkill[5] = 56; // Secondary skill if monster is immune to primary. 이뮨이 있는 몹 스킬 쿨타임 있는 스킬
	...
	/* Advanced Attack config. Allows custom skills to be used on custom monsters.	### 툭정 보스에게 사용 할 스킬
	...
	// 몹과의 거리 유지 설정
	Config.NoTele = false; // Restrict char from teleporting. Useful for low level/low mana chars ### 텔레포트 하지 않고 걸어서 이동
	Config.Dodge = true; // Move away from monsters that get too close. Don't use with short-ranged attacks like Poison Dagger. ### 너무 가까이 다가오는 몬스터들로부터 멀리 떨어지십시오.
	Config.DodgeRange = 10; // Distance to keep from monsters. ### 몬스터와의 거리.
	Config.DodgeHP = 100; // Dodge only if HP percent is less than or equal to Config.DodgeHP. 100 = always dodge.
	Config.BossPriority = false; // Set to true to attack Unique/SuperUnique monsters first when clearing	###	클리어시 유니크 / 슈퍼 유니크 몬스터를 먼저 공격하려면 참으로 설정
	Config.ClearType = 0xF; // Monster spectype to kill in level clear scripts (ie. Mausoleum). 0xF = skip normal, 0x7 = champions/bosses, 0 = all
	Config.TeleStomp = true; // Use merc to attack bosses if they're immune to attacks, but not to physical damage 보스가 공격에 면역이지만 물리적 손상에는 영향을 미치지 않는 경우 용병을 사용하여 보스를 공격하십시오.
	...
	// Class specific config 보스몹에게 스태틱필드 시전할 범위
